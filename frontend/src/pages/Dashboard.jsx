...
import { ExternalLink, Trash2, BarChart2, Calendar, MousePointer2, Copy, Check } from 'lucide-react';
    ...
    <div className="max-w-6xl mx-auto px-4 py-12 space-y-12">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-base-content/60 font-medium">Manage and track your shortened links</p>
        </div>
        <div className="stats shadow bg-base-100 border border-base-content/10 rounded-2xl">
          <div className="stat px-8">
            <div className="stat-title text-xs uppercase tracking-widest font-bold">Total Links</div>
      ...
      {urls.length === 0 ? (
        <div className="p-20 text-center bg-base-200/50 rounded-[3rem] border-2 border-dashed border-base-content/10">
          <p className="text-lg font-medium opacity-50">No links found. Start by shortening one!</p>
        </div>
      ) : (
          ...
          {urls.map((url, idx) => (
            ...
            >
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                <div className="flex-1 min-w-0 space-y-1">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-xs font-mono opacity-40">#{url.short_code}</span>
                  </div>
                ...
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  >
                    {copiedId === url.short_code ? <Check size={16} /> : <Copy size={16} />}
              ...
              <div className="mt-6 pt-6 border-t border-base-content/5 grid grid-cols-2 md:grid-cols-4 gap-4">
                  <StatMini icon={<MousePointer2 size={14}/>} label="Clicks" value="--" />
                  <StatMini icon={<BarChart2 size={14}/>} label="Top Region" value="--" />
                  <StatMini icon={<Calendar size={14}/>} label="Created" value="New" />
              </div>
            </motion.div>
          ))}
        </div>
